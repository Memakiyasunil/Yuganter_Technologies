      ...
      <div className="flex-1 p-4 md:p-6 lg:p-8">
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-[#071d34] rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto my-8">
              {/* Modal Header */}
              <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 md:p-6 flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-2xl font-bold hover:text-slate-200"
                >
                  ✕
                </button>
              </div>
        ...
        <div className="md:hidden space-y-4">
          {students.length === 0 ? (
            <div className="bg-[#071d34] rounded-lg p-8 text-center">
            ...
            students.map((s) => (
              <div key={s._id} className="bg-[#071d34] rounded-lg shadow-sm hover:shadow-md transition-shadow p-4 border-l-4 border-blue-500">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <p className="font-bold text-white text-lg">{s.name}</p>
                    <p className="text-sm text-gray-600">{s.course}</p>
                  </div>
                  <span className={`text-xs px-3 py-1 rounded-lg font-semibold ${s.type === 'Internship' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                    {s.type}
                  </span>
                </div>
        ...
        {/* DESKTOP TABLE */}
        <div className="hidden md:block bg-[#071d34] rounded-lg shadow-md overflow-hidden">
          {students.length === 0 ? (
                  ...
                  {students.map((s) => (
                    <tr key={s._id} className="hover:bg-blue-50 transition-colors">
                      ...
                      <td className="p-4 text-center">
                        <span className={`text-xs px-3 py-1 rounded-lg font-bold ${s.type === 'Internship' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                          {s.type}
                        </span>
                      </td>
                      <td className="p-4 text-center">
